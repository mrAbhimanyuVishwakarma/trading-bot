...
async def make_decision(
    ...
    3. Combine scores → BUY / SELL / HOLD
    ...
    if tech["direction"] == "BUY" and sentiment == "positive" and sent_prob >= SENTIMENT_BUY_THRESHOLD:
        decision = "BUY"
    elif tech["direction"] == "SELL" and sentiment == "negative" and sent_prob >= SENTIMENT_SELL_THRESHOLD:
        decision = "SELL"
    else:
        logger.info(f"HOLD: tech={tech['direction']} | sentiment={sentiment} ({sent_prob:.2%})")
